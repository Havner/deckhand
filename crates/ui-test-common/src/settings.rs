...
//! profile paths) and, in the real app, the launch behaviour. The test only reads/writes them; it
//! does not act on `start_daemon` yet.
...
pub struct AppSettings {
    ...
    /// The UI theme, stored by **name** (e.g. `Dark`, `Dracula`). Toolkit-independent: each
    /// `ui-test-*` crate maps the name onto one of its own built-in themes, falling back to its
    /// default when it doesn't recognize the name.
    pub theme: String,
}
...
/// Where the UI settings live: `$XDG_CONFIG_HOME/deckhand/ui-test.ron` (fallback `~/.config/…`) on
/// unix, `%APPDATA%\deckhand\ui-test.ron` on Windows.
pub fn settings_path() -> PathBuf {
    let dir = config_dir().join("deckhand");
    dir.join("ui-test.ron")
}
