//! The config-facing core of the output vocabulary: the leaf target enums a binding can name -
//! [`Key`], [`MouseButton`], [`GamepadButton`], [`GamepadAxis`] (plus [`SCROLL_HI_RES_PER_DETENT`]).
//! This is the part `config` needs. The [`OutputEvent`](crate::OutputEvent) batch item the mapper
//! emits (built from these) lives in `event`. No platform deps: each backend maps these bare names
//! to OS codes (`virt-out` Linux -> evdev, Windows -> scancode/VK).
