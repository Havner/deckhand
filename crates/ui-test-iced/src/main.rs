//! `ui-test-iced` — the iced implementation of the deckhand UI-toolkit bake-off.
//!
//! The reference (retained-mode) implementation of the shared layout: a four-region window
//! (top daemon bar / left sidebar / scrollable content / bottom status bar) driven by the real
//! daemon over `ipc`. Everything toolkit-independent lives in `ui-test-common`; this crate is only
//! the iced widget layer + the Elm-style state/update/view wiring.
...
fn main() -> iced::Result {
    iced::application(App::new, App::update, App::view)
        .title("deckhand — UI toolkit test (iced)")
...
impl App {
    ...
    /// empty/unknown name — e.g. a name another toolkit wrote to the shared settings file).
