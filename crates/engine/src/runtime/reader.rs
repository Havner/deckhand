...
fn read_session(
    ...
    // The in-flight command-feedback sequence, if any (PLAN 7.4); replaced latest-wins per request.
    let mut feedback: Option<feedback::Sequencer> = None;
...
/// ([`RUMBLE_MAX_DUTY`]) at us resolution, so even a fraction-of-a-percent effective strength
...
fn train(drive: u16, hz: u16) -> HapticPulse {
    ...
    let period = 1_000_000u32 / hz as u32; // us
    let full = drive as f32 / u16::MAX as f32;
    // Full drive -> MAX_DUTY of the period; keep the us precision so tiny drives stay tiny. A
...
mod tests {
    ...
    fn train_maps_full_drive_to_the_max_duty_band() {
        ...
        // A fraction-of-a-percent effective drive still yields a distinct, small pulse (us
