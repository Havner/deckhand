...
//!   ringdown          fixed pitch x rising duration, and above-resonance ring-down tail
...
fn tone(dev: &mut Device, motor: HapticPosition, freq_hz: u32, ms: u32) -> Result<()> {
    ...
    let half = (500_000 / freq_hz).clamp(1, u16::MAX as u32) as u16; // half-period us
...
fn main() -> Result<()> {
    ...
    match positional.first().map(String::as_str) {
        ...
        // Candidate feedback PATTERNS built from the portable levers (pitch level x count x length x
