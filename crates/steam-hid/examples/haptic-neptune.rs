...
//!   clicks                    0xEA clicks - side x type (Tick/Click) x ui_intensity
...
//!   clicks-gain               0xEA Click: side x ui_intensity(System/Long) x gain (-8,0,8,16 dB)
...
fn main() -> Result<()> {
    ...
    match mode {
        ...
        // beats a full 0x8F click). side (LEFT/RIGHT/BOTH - 0xEA honors a native BOTH) x type
        // (Tick, Click - Off skipped, it's silent) x every ui_intensity (System..Insane; HW: 0..2
        ...
        Some("clicks") => {
            ...
            println!("0xEA clicks: side x type x ui_intensity (gain=0):");
        ...
        Some("clicks-gain") => {
            println!("0xEA CLICK gain sweep: side x ui_intensity(System/Long) x gain (dB):");
            for (pad, side) in [("LEFT ", HapticSide::Left), ("RIGHT", HapticSide::Right), ("BOTH ", HapticSide::Both)] {
                if !running.alive() {
                    break;
                }
