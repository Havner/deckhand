...
//!
//! The `0x8F` pulse is a square wave on a trackpad actuator: each cycle is `duration` us ON then
//! `interval` us OFF, repeated `count` times, so **frequency ~ 1e6/(dur+interval) Hz**, the on/off
    ...
    match mode {
        ...
        Some("clicks") => {
            ...
            for (side, wire) in [("LEFT", Some(1u8)), ("RIGHT", Some(0u8)), ("BOTH", None)] {
                ...
                for (name, dur, interval, count) in clicks {
                    ...
                    println!("    {name} (dur={dur}us interval={interval}us count={count})");
                    keep_lizard_off(&mut device);
                    match wire {
        ...
        Some("duty") => {
            ...
            for (label, left, right) in [("LEFT ", true, false), ("RIGHT", false, true), ("BOTH ", true, true)] {
                ...
                for pct in [1u32, 2, 3, 5, 8, 12, 16, 20, 25, 30, 50, 100] {
                    ...
                    let interval = period - dur;
                    println!("    {pct:>3}%  (dur={dur}us interval={interval}us count={count})");
                    keep_lizard_off(&mut device);
                    let p = HapticPulse { duration: dur, interval, count, gain: 0 };
        ...
        Some("freq") => {
            ...
            for hz in [25u32, 40, 60, 90, 130, 200, 350, 600, 1000] {
                ...
                let count = ((hz * TRAIN_MS) / 1000).max(1) as u16;
                println!("  {hz:>4} Hz  (dur={half}us interval={half}us count={count})");
                keep_lizard_off(&mut device);
        ...
        Some("custom") => {
            ...
            println!(
                "custom: pad={pad} dur={dur}us interval={interval}us count={count} (~{freq} Hz, ~{}ms)",
