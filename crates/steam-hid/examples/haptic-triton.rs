...
//!   clicks                    0x82 command/click — side × style (Weak/Strong) × amplitude
...
fn main() -> Result<()> {
    ...
    match mode {
        ...
        // 0x82 command/click — side × style (Weak/Strong = the main strength lever) × amplitude
        // (unsigned trim, 0=medium..255=strong; largely inert). BOTH honored.
        Some("clicks") => {
            println!("0x82 command/click: side × style × amplitude:");
        ...
        Some("pulse") => {
            ...
            for (label, side) in [("LEFT ", HapticSide::Right), ("RIGHT", HapticSide::Left), ("BOTH ", HapticSide::Both)] {
                ...
                for hz in [20u32, 40, 60, 90, 130, 200, 350, 600, 1000, 1500] {
                    ...
                    println!("    {hz:>4} Hz  (on={half}µs off={half}µs count={count})");
                    keep_lizard_off(&mut device);
        ...
        Some("clicks-pulse") => {
            ...
            println!("0x81 single-pulse clicks (repeat_count=1, on_us=strength): side × strength:");
            for (label, side) in [("LEFT ", HapticSide::Left), ("RIGHT", HapticSide::Right), ("BOTH ", HapticSide::Both)] {
                if !running.alive() {
                    break;
                }
